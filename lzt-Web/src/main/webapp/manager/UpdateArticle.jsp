...
<title>Insert title here</title>
    ...
    <script type="text/javascript">
       ...
       function publish(){
           if($("#title").val().trim() == ""){
               $.messager.alert('警告','请输入标题');
               return false;
           }
           if($("#type").val() == 0){
               $.messager.alert('警告','请选择分类');
               return false;
           }
           if(UE.getEditor('editor').getContentTxt().trim() == ""){
               $.messager.alert('警告','正文内容不能为空');
               return false;
           }
               ...
               function(result){
                  if(result.code=='000000'){
                      $.messager.alert('我的消息','修改成功！','info',function(){
                         window.parent.closeTab("修改文章"+$("#aid").val());
                         // window.parent.openTab('文章列表','articleList.jsp','icon-bklb');
                      });
                  }else{
                      $.messager.show({
                          title:'我的消息',
       ...
       function closeTab(id){
           window.parent.closeTab("修改文章"+id);
       }
    </script>
</head>
<body>
