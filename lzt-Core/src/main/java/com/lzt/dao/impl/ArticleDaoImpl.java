...
public class ArticleDaoImpl extends AllDaoImpl<Article> implements ArticleDao {
	...
	public Article saveArt(Article art) {
		...
		if(art.getAid()!=null){
			String sql = " update lzt_article a set a.content=:content,content_text=:contentText," +
					"sign=:sign,title=:title,typeId=:typeId,updateTime=:updateTime where id=:id";
			Query query = this.getHibernateTemplate().getSessionFactory().getCurrentSession().createSQLQuery(sql)
					.setParameter("content",art.getContent()).setParameter("contentText",art.getContentText())
					.setParameter("sign",art.getSign()).setParameter("title",art.getTitle())
	...
	public Map<String, Object> getPageArticleList(Integer start, Integer pageSize) {
		String sql = "SELECT a.id id,a.`title` title,a.`content_text` content,a.`createTime` createTime,a.`updateTime` updateTime,b.`type_name` typeName,c.`user_name` userName \n" +
				"FROM `lzt_article` a,`lzt_type` b,`lzt_user` c WHERE a.`userId`=c.`id` AND a.`typeId`=b.`id` and a.status=0 ORDER BY a.`updateTime` desc";
		Query query = this.getHibernateTemplate().getSessionFactory().getCurrentSession().createSQLQuery(sql)
				...
				.addScalar("typeName",StringType.INSTANCE).addScalar("userName",StringType.INSTANCE)
	...
	public Integer deleteById(List<Integer> idList) {
		String hql = "update Article art set art.status=1,art.updateTime=:updateTime where art.id in (:idList)";
		Query query = this.getHibernateTemplate().getSessionFactory().getCurrentSession().createQuery(hql)
